...
const CATEGORIES = ["Leather", "Watches", "Jewellery", "Ready-to-wear", "Vintage"];
...
export function CollectionClient() {
  ...
  const filtered =
    filter === "all"
      ? PRODUCTS
      : filter === "vault"
        ? PRODUCTS.filter((p) => p.stockType === "in-vault")
        : PRODUCTS.filter((p) => p.stockType === "estimated");
  ...
  return (
    <div className="fade-in">
      <div className="shell" style={{ paddingTop: 48, paddingBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 32, flexWrap: "wrap" }}>
          <div>
            ...
            <h1
              className="display"
              style={{ fontSize: 64, marginTop: 16, lineHeight: 1, letterSpacing: "-0.02em" }}
            >
              Eighty-two pieces
              <br />
              currently <span style={{ fontStyle: "italic", color: "var(--accent)" }}>open to file</span>.
            </h1>
          </div>
          <div style={{ maxWidth: 320, color: "var(--ink-2)", fontSize: 13.5 }}>
            Every figure converts at <span className="mono">{CCY[ccy].sym}</span> ·
      ...
      <div className="shell" style={{ paddingBottom: 24 }}>
        ...
        <div className="row-between" style={{ padding: "20px 0", flexWrap: "wrap", gap: 14 }}>
          <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
            ...
            <span
              className="mono"
              style={{ color: "var(--ink-3)", fontSize: 10, letterSpacing: "0.14em", marginLeft: 18 }}
            >
              CATEGORIES —
            </span>
            {CATEGORIES.map((c) => (
              <span key={c} className="tag" style={{ cursor: "pointer" }}>
                {c}
              </span>
            ))}
          </div>
          <div className="row">
            <span className="mono" style={{ color: "var(--ink-3)", fontSize: 10, letterSpacing: "0.14em" }}>
              SORT
            </span>
            <select className="input-sm" style={{ paddingRight: 28 }} defaultValue="newest">
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 32, alignItems: "center" }}>
            <div>
              ...
              <h3
                className="display"
                style={{ fontSize: 32, marginTop: 14, fontWeight: 400, letterSpacing: "-0.015em" }}
              >
                Send us a photo, a link, or simply{" "}
                <em style={{ fontStyle: "italic", color: "var(--accent)" }}>describe it.</em>
              </h3>
